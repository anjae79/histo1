document.addEventListener('DOMContentLoaded', function() {
    ...
    const serviceOptions = {
        'paraffin': ['Block제작', 'Only section', 'Only H&E', 'Only section & H&E', 'Block & section', 'Block & section & H&E'],
        'frozen': ['Block제작', 'Only section', 'Only H&E', 'Only section & H&E', 'Block & section', 'Block & section & H&E'],
    ...
    function addServiceToTable() {
        ...
        row.innerHTML = `
            ...
            <td>${slideCount}</td>
            <td>${totalCount}</td>
            <td><button type="button" class="remove-service" onclick="removeServiceRow(this)">삭제</button></td>
        `;
...
window.collectFormData = function() {
    ...
    serviceRows.forEach((row, index) => {
        ...
        // 특수염색인 경우 특수염색 종류와 세부분석을 결합
        let antibodyType = '';
        if (mainService === 'special' && specialType && subService) {
            antibodyType = `${specialType} (${subService})`;
        } else if (specialType) {
            antibodyType = specialType;
        } else if (subService) {
            antibodyType = subService;
        }
        data[`antibody_type_${serviceIndex}`] = antibodyType;
        
        data[`slide_count_${serviceIndex}`] = slideCount;
        data[`total_count_${serviceIndex}`] = totalCount;
        
        // 추가 디버깅 정보
        console.log(`설정된 데이터 ${serviceIndex}:`, {
